...
eff_data = np.genfromtxt("cali_eff_Ne21_0618.csv",dtype=float, delimiter="\t")
...
for i in range (6) :
	...
	fit_parameters = np.polyfit (gamma_energy, eff, 7)
	uncertainty = eff * np.sqrt( ((I_error / Intensity)**2) + ((eff_data[:,error_index]/eff_data[:,index])**2))
	
	ax[coor[i]].plot(gamma_energy, eff , color='k',marker="o",linestyle="None",markersize=3, label=name[i])
	#ax[coor[i]].loglog(gamma_energy, eff)
	ax[coor[i]].errorbar(gamma_energy, eff, yerr = uncertainty , xerr = None, ecolor = 'b',linestyle='none')
	ax[coor[i]].set_xlim (800, 3500)
	ax[coor[i]].set_ylim ()
	p = np.poly1d (fit_parameters)

	chisquare = lambda e, model, sigma: sum(((e - model)/sigma)**2) / (10 - 1)

	print ("--> 1369 efficiency of ", name[i]," is", p(1369))
	print ("fitting parametes", fit_parameters)
	print ("chi square", chisquare (eff, p(gamma_energy), uncertainty))
